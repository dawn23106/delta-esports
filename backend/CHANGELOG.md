...
- **统一 Redis 抽象**：`RedisCacheTemplate`（key 前缀/JSON 序列化/fail-open/分布式锁），各 Service 缓存收敛到统一入口
- **WebSocket 跨实例广播**：Redis Pub/Sub（channel `delta:orders:push`），多实例部署推送不丢失，未启用时本地直推
- **定时任务多实例互斥**：订单超时清理加 Redis 分布式锁，避免多实例重复扫描
- **封禁即时生效**：`JwtInterceptor` 校验用户状态（Redis 缓存+主动失效），被封禁用户立即 403
